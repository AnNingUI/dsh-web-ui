...
/** Synthesize the registry entry for an EXTERNAL-scope skin package name. */
function syntheticEntry(packageName: string): SkinCenterEntry | undefined {
  const match = SKIN_PACKAGE_ID.exec(packageName)
...
/** Resolve a package name to a skin entry: prebuilt registry first, external synthesis second. */
function entryForPackage(packageName: string): SkinCenterEntry | undefined {
  return SKIN_CENTER_ENTRIES.find(entry => entry.package === packageName) ?? syntheticEntry(packageName)
}

/**
 * The skin package currently ACTIVE in the boot graph, if it is one of ours.
 * Matches each boot entry id against the prebuilt registry first (known skins
...
function bootSkinEntry(): SkinCenterEntry | undefined {
  ...
  for (const id of ids) {
    const entry = entryForPackage(id)
...
export function activeSkinEntry(): SkinCenterEntry | undefined {
  if (hotOverride !== undefined) {
    return hotOverride.pkg === null
      ? undefined
      : entryForPackage(hotOverride.pkg)
  }
