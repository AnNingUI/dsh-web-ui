...
*/
function profilePatchInsertsId(profilePatchPath, id) {
	if (profilePatchPath === void 0) return false;
	let patch;
	try {
...
* skins.
*/
function registryWithProfileWiring(registry, profileModulesDir, profileManifestPath, profilePatchPath) {
	const out = {};
	for (const [name, entry] of Object.entries(registry)) out[name] = activeSkinIsBundleWired(entry, profileModulesDir, profileManifestPath) || profilePatchInsertsId(profilePatchPath, entry.id) ? {
		...entry,
		bundleWired: true
	} : entry;
	return out;
...
function resolvePaths(home, profile, fromUrl = import.meta.url) {
	...
	return {
		...
		profilePatchPath: join(harnessHome, "profiles", activeProfile, "cordis.patch.yml")
	};
}
